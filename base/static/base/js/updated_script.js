...
function updateFontSize() {
  ...
  // Calculate the new font size based on viewport width (VW)
  const screenWidth = window.innerWidth;
  const newSize = (screenWidth / 100) * 1.5; // Adjust 1.5 as needed for scaling sensitivity
  
  // Clamp the font size between min and max
  const clampedSize = Math.max(minFontSize, Math.min(newSize, maxFontSize));

  // Use GSAP to animate the font size change smoothly
  gsap.to(":root", {
    duration: 0.3,                // Animation duration (seconds)
    "--size-font": `${clampedSize}px`, // Set the CSS variable
    ease: "power1.out"            // Smooth easing transition
  });
}

// Initial call and attach the function to the resize event
window.addEventListener('resize', updateFontSize);
updateFontSize(); // Initial execution
